...
有用户想要如上图的效果，能够使用自己的模板来发送消息，并且能够控制模板中的关键词内容。这种情况下，使用普通消息模板是不行的，因为普通消息中只有title和content两个字段。可以使用其他消息模板来实现，比如“支付成功通知”。

![消息模板](../images/mp-3.png)

在模板设置中，插入变量可以选择更多变量值，不用管变量叫什么，这只是提前取了个名字，具体变量值的内容是你在发送消息内容里面填写的。

![配置模板](../images/2.png)

然后在发送消息的时候，就需要指定template参数为设置的模板，比如“pay”。完整的请求报文如下：
...
#### 总结
1. 本质是普通消息模板中content参数是一个字符串，不会像其他模板，把content参数认为是一个对象，可以进一步指定对象下的属性。
2. 这种方式只是修改微信模板消息中的内容，点击消息后消息详情中的格式是跟着模板定制的，无法直接修改。
3. 其他模板的发送报文，可以在[消息模板枚举](../template/README.md)中的使用教程中找到。
