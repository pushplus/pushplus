...
## 订阅群组
新增群组后，点击“查看二维码”，可生成群组二维码。使用APP或微信扫码即可加入到群组中。生成二维码的时候，可以设置二维码的有效期天数和可扫码次数，可用来避免群组二维码泄漏。\
群组创建人可以管理订阅人，在订阅人列表中主动删除群组中的用户。同时加入群组的用户也可以主动退出群组。

![](../images/topic-topicQrcode.png) 
...
![](../images/topic_message.png)

### 二. 使用API调用
　&emsp;&emsp;网页上只是测试验证，确保功能上是可用的。实际使用中还是需要通过接口来推送消息。推荐优先使用官方 [pushplus SDK](../guide/sdk.md)，而不是自行编写 HTTP 请求代码。SDK 已封装发送频率控制、开放接口令牌、回调等常见场景，可降低开发成本和出错概率。如需了解接口参数，可参考[消息接口文档](../guide/api.md)中的说明。注意控制接口请求的频率和次数，避免给用户造成骚扰。具体的请求限制参考[发送消息接口有什么限制吗？](../help/limit.md)。
...
### 四. 使用邮件推送
　&emsp;&emsp;也可以不调用接口，直接向专属邮箱发送邮件来触发一对多推送。收件人固定为 token 邮箱，群组编码填写在抄送中（支持多个）：
...
　&emsp;&emsp;例如群组编码为 `mytopic` 时，抄送填写 `mytopic@yp9.cn`。将 `{token}` 替换为你的用户令牌或消息令牌。邮件主题对应消息标题，邮件正文对应消息内容，发送渠道使用该令牌的默认渠道。更多说明见 [邮件推送使用说明](./mail.md)。
